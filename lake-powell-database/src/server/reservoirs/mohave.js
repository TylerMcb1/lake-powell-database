...
router.get('/', async (req, res) => {
    try {
        const db = getDB();
        let collection = await db.collection('LakeMohave');
...
router.get('/weather', async(req, res) => {
    ...
    try {
        const response = await axios.get('https://api.weather.gov/gridpoints/VEF/135,63/forecast/hourly?units=us', {
...
router.get('/sunrise-sunset', async(req, res) => {
    ...
    try {
        const response = await axios.get('https://api.sunrise-sunset.org/json?lat=35.440&lng=-114.651&formatted=0', {
            headers: { 'User-Agent': 'ColoradoRiverData/1.0 (ColoradoRiverData@gmail.com)' },
            timeout: APITIMEOUT,
        });
